...
const Projects = () => {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: getProjects,
    select: (res) =>
      res.data.filter((project) => project.projectType === projectType),
    retry: false,
  });
  ...
  if (data) {
    ...
    return (
      <motion.main
        ...
        className="flex flex-col z-1 w-full min-h-full max-w-240 px-4 md:px-8 lg:px-16 pt-6 md:pt-10"
      >
        <>
          {/* Title Section */}
          ...
          {/* Projects Grid */}
          {data.length === 0 ? ((() => {
            const uppercaseProjectType = projectType.charAt(0).toUpperCase() + projectType.slice(1);
            return <ComingSoon title={`${uppercaseProjectType} Projects`} subtitle={"check back soon."} />
            })()
          ) : (
